...
def homepage():
    ...
    return render_template('index.html', orders=orders, clients=clients)
...
def order_details(id):
    ...
    return render_template('order.html', order_object=order_object)

@app.route("/clients")
def clients():
    clients_query = db.session.execute(db.select(Client)).scalars()
    return render_template('clients.html', client_query=clients_query)
